...
static void getinputArgs( config *cfg, char pCh, char *inFName, char *outFNamem, int wSize, int lSize)
{
    cfg->window_sz2 = DEF_WINDOW_SZ2;
    cfg->lookahead_sz2 = DEF_LOOKAHEAD_SZ2;
    cfg->buffer_size = DEF_BUFFER_SIZE;
    ...
    cfg->in_fname = malloc(sizeof(char) * (strlen(inFName) + 1)); //Allocate memory
    if (!cfg->in_fname) {
        /* Error handling */
    } else
    {
        strcpy(cfg->in_fname, inFName);
    }
    cfg->out_fname = malloc(sizeof(char) * (strlen(outFNamem) + 1)); //Allocate memory
    if (!cfg->out_fname) {
        /* Error handling */
    } else
    {
        strcpy(cfg->out_fname, outFNamem);
    }
    //strcpy1(cfg->in_fname,inFName);
    //strcpy1(cfg->out_fname,outFNamem);
