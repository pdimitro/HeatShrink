...
{
    clock_t start = clock();
	//Save the output file to original sound file...Decoding..
	(void)mainHeatShrink('d', DecodedSounds[soundId], OriginalSounds[soundId], 12, 6);

    clock_t end = clock();

    float msseconds = (float)(end - start) ;
    printf("\nThe decoding of sound elapsed time is %f ms", msseconds);
    //Play the sound...
    system(OriginalSounds[soundId]);

    Sleep(200);
    //REMOVE the original sound file from the RAM memory
